...
//     var_dump($clienteSOAP->__getFunctions());
  ...
  function download($type,$range,$force){
    $encuesta = null;
    $this->force = json_decode(base64_decode($force),true);
    ...
    if ($encuesta){
      ...
      return Response::download($file, $type.'.xml', $headers);
    } 
  ...
  function showData(Request $request){
    ...
    if(isset($encuesta['HABITACIONES'])){
      ...
      foreach ($obj as $k=>$v){
        $movApart[0][] = $v['PLAZAS_SUPLETORIAS'];
        $movApart[1][] = $v['HABITACIONES_DOBLES_USO_DOBLE'];
        $movApart[2][] = $v['HABITACIONES_DOBLES_USO_INDIVIDUAL'];
      }
