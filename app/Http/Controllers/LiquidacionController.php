<?php
	...
	class LiquidacionController extends Controller
	{
		...
		{
			$now         = Carbon::now();
			$totales     = ["total" => 0 , "coste" => 0 , "bancoJorge" => 0 , "bancoJaime" => 0 , "jorge" => 0 , "jaime" => 0 , "costeApto" => 0 , "costePark" => 0 , "costeLujo" => 0 , "costeLimp" => 0 , "costeAgencia" => 0 , "benJorge" => 0 , "benJaime" => 0 , "pendiente" => 0 , "limpieza" => 0 , "beneficio" => 0 , "stripe" => 0 , "obs" => 0 ,];
			...
			$books = \App\Book::with( ['customer' , 'payments' , 'room.type'] )->where( 'start' , '>=' , $date )->where( 'start' , '<=' , $date->copy()->addYear()->subMonth() )->whereIn( 'type_book' , [2 , 7 , 8] )->orderBy( 'start' , 'ASC' )->get();
			...
			$data = ['days-ocupation' => 0 , 'total-days-season' => \App\SeasonDays::first()->numDays , 'num-pax' => 0 , 'estancia-media' => 0 , 'pax-media' => 0 , 'precio-dia-media' => 0 , 'dias-propios' => $countDiasPropios , 'agencia' => 0 , 'propios' => 0 ,];
			...
			$mobile = new Mobile();
			if ( !$mobile->isMobile() ) {
				return view( 'backend/sales/index' , ['books' => $books , 'totales' => $totales , 'temporada' => $date , 'data' => $data , 'percentBenef' => DB::table( 'percent' )->find( 1 )->percent ,] );
			} else {
				return view( 'backend/sales/index' , ['books' => $books , 'totales' => $totales , 'temporada' => $date , 'data' => $data , 'percentBenef' => DB::table( 'percent' )->find( 1 )->percent ,] );
			}
		}
		...
		public function apto( $year = "" )
		{
			...
			$rooms        = \App\Rooms::all();
			$pendientes   = array();
			$apartamentos = ["room" => [] , "noches" => [] , "pvp" => [] , "pendiente" => [] , "beneficio" => [] , "%ben" => [] , "costes" => [] ,];
			...
			return view( 'backend/sales/liquidacion_apto' , ['rooms' => $rooms , 'apartamentos' => $apartamentos , 'temporada' => $date , 'pendientes' => $pendientes , 'percentBenef' => DB::table( 'percent' )->find( 1 )->percent ,] );
		}
		...
		public function contabilidad( $year = "" )
		{
			...
			return view( 'backend/sales/contabilidad' , ['date' => $date , 'inicio' => $inicio , 'arrayTotales' => $arrayTotales , 'rooms' => $rooms , 'priceBookRoom' => $priceBookRoom ,] );
		}
		...
		public function gastos( $year = "" )
		{
			...
			$gastos = \App\Expenses::where( 'date' , '>=' , $inicio->copy()->format( 'Y-m-d' ) )->Where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->where( 'concept' , 'NOT LIKE' , '%LIMPIEZA RESERVA PROPIETARIO.%' )->orderBy( 'date' , 'DESC' )->get();

			$books           = \App\Book::whereIn( 'type_book' , [2] )->where( 'start' , '>' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'start' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'start' , 'ASC' )->get();
			$totalStripep    = 0;
			$comisionBooking = 0;
			$obsequios       = 0;
			foreach ( $books as $key => $book ) {
			...
			return view( 'backend/sales/gastos/gastos' , ['date' => $date , 'inicio' => $inicio , 'gastos' => $gastos , 'totalStripep' => $totalStripep , 'comisionBooking' => $comisionBooking , 'obsequios' => $obsequios , 'year' => $inicio] );
		}
		...
		public function getTableGastos( $year = '' )
		{
			...
			$gastos = \App\Expenses::where( 'date' , '>=' , $inicio->copy()->format( 'Y-m-d' ) )->Where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'DESC' )->get();
			...
			return view( 'backend/sales/gastos/_tableExpenses' , ['gastos' => $gastos , 'totalStripep' => $totalStripep , 'comisionBooking' => $comisionBooking , 'obsequios' => $obsequios ,] );
		}
		...
		public function ingresos( $year = "" )
		{
			...
			foreach ( $conceptIncomes as $typeIncome ) {
				$incomes = \App\Incomes::where( 'concept' , $typeIncome )->where( 'date' , '>' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->get();
			...
			return view( 'backend/sales/ingresos/ingresos' , ['inicio' => $inicio , 'arrayTotales' => $arrayTotales , 'incomes' => $arrayIncomes ,] );
		}
		...
		public function caja( $year = "" )
		{
			...
			$cashJaime    = \App\Cashbox::where( 'typePayment' , 1 )->where( 'date' , '>=' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'ASC' )->get();
			$saldoInicial = \App\Cashbox::where( 'concept' , 'SALDO INICIAL' )->where( 'typePayment' , 1 )->first();

			$cashJorge = \App\Cashbox::where( 'typePayment' , 0 )->where( 'date' , '>' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->get();

			return view( 'backend.sales.cashbox.cashbox' , ['inicio' => $inicio , 'cashJaime' => $cashJaime , 'cashboxJor' => $cashJorge , 'saldoInicial' => $saldoInicial ,] );
		}
		...
		public function getTableMoves( $year , $type )
		{
			...
			if ( $type == 'jaime' ) {
				...
				$cashbox      = \App\Cashbox::where( 'typePayment' , 1 )->where( 'date' , '>=' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'ASC' )->get();
				$saldoInicial = \App\Cashbox::where( 'concept' , 'SALDO INICIAL' )->where( 'typePayment' , 1 )->first();

			} else {
				$cashbox = \App\Cashbox::where( 'typePayment' , 0 )->where( 'date' , '>=' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'ASC' )->get();

				$saldoInicial = \App\Cashbox::where( 'concept' , 'SALDO INICIAL' )->where( 'typePayment' , 0 )->first();

			}
			return view( 'backend.sales.cashbox._tableMoves' , ['cashbox' => $cashbox , 'saldoInicial' => $saldoInicial ,] );
		}
		...
		public function bank( $year = "" )
		{
			...
			$bankJaime    = \App\Bank::where( 'typePayment' , 3 )->where( 'date' , '>=' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'ASC' )->get();
			$saldoInicial = \App\Bank::where( 'concept' , 'SALDO INICIAL' )->where( 'typePayment' , 3 )->first();

			$bankJorge = \App\Bank::whereIn( 'typePayment' , [2 , 0] )->where( 'date' , '>' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'ASC' )->get();

			return view( 'backend.sales.bank.bank' , ['inicio' => $inicio , 'bankJaime' => $bankJaime , 'bankJorge' => $bankJorge , 'saldoInicial' => $saldoInicial ,] );
		}
		...
		public function getTableMovesBank( $year , $type )
		{
			...
			if ( $type == 'jaime' ) {
				...
				$bank         = \App\Bank::where( 'typePayment' , 3 )->where( 'date' , '>=' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'ASC' )->get();
				$saldoInicial = \App\Bank::where( 'concept' , 'SALDO INICIAL' )->where( 'typePayment' , 3 )->first();

			} else {
				$bank = \App\Bank::where( 'typePayment' , 2 )->where( 'date' , '>=' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'date' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'ASC' )->get();

				$saldoInicial = \App\Bank::where( 'concept' , 'SALDO INICIAL' )->where( 'typePayment' , 2 )->first();

			}
			return view( 'backend.sales.bank._tableMoves' , ['bank' => $bank , 'saldoInicial' => $saldoInicial ,] );
		}
		...
		public function perdidasGanancias( $year = "" )
		{
			...
			$books = \App\Book::whereIn( 'type_book' , [2 , 7 , 8] )->where( 'start' , '>' , $inicio->copy()->format( 'Y-m-d' ) )->where( 'start' , '<=' , $inicio->copy()->addYear()->format( 'Y-m-d' ) )->get();
			/* INGRESOS */
			$arrayTotales = ['totales' => 0 , 'meses' => []];
			...
			$conceptExpenses = ['PAGO PROPIETARIO' , 'AGENCIAS' , 'STRIPE' , 'SERVICIOS PROF INDEPENDIENTES' , 'VARIOS' , 'REGALO BIENVENIDA' , 'LAVANDERIA' , 'LIMPIEZA' , 'EQUIPAMIENTO VIVIENDA' , 'DECORACION' , 'MENAJE' , 'SABANAS Y TOALLAS' , 'IMPUESTOS' , 'GASTOS BANCARIOS' , 'MARKETING Y PUBLICIDAD' , 'REPARACION Y CONSERVACION' , 'SUELDOS Y SALARIOS' , 'SEG SOCIALES' , 'MENSAJERIA' , 'COMISIONES COMERCIALES'];
			...
			return view( 'backend/sales/perdidas_ganancias' , ['arrayTotales' => $arrayTotales , 'arrayIncomes' => $arrayIncomes , 'arrayExpenses' => $arrayExpenses , 'inicio' => $inicio , 'arrayExpensesPending' => $arrayExpensesPending , 'selectedYear' => empty( $year ) ? (date( 'Y' ) - 1) : $year->format( 'Y' ) ,] );
		}
		...
		static function getSalesByYear( $year = "" )
		{
			...
			$books = \App\Book::with( 'payments' )->whereIn( 'type_book' , [2 , 7 , 8] )->where( 'start' , '>' , $start->copy()->format( 'Y-m-d' ) )->where( 'start' , '<=' , $end->copy()->format( 'Y-m-d' ) )->orderBy( 'start' , 'ASC' )->get();
		...
		static function getSalesByYearByRoom( $year = "" , $room = "all" )
		{
			...
			if ( $room == "all" ) {
				...
				$books = \App\Book::whereIn( 'type_book' , [2] )->whereIn( 'room_id' , $rooms )->where( 'start' , '>=' , $start->copy()->format( 'Y-m-d' ) )->where( 'start' , '<=' , $end->copy()->format( 'Y-m-d' ) )->orderBy( 'start' , 'ASC' )->get();

				$gastos = \App\Expenses::where( 'date' , '>=' , $start->copy()->format( 'Y-m-d' ) )->Where( 'date' , '<=' , $end->copy()->format( 'Y-m-d' ) )->orderBy( 'date' , 'DESC' )->get();

			} else {

				$books = \App\Book::whereIn( 'type_book' , [2] )->where( 'room_id' , $room )->where( 'start' , '>=' , $start->copy()->format( 'Y-m-d' ) )->where( 'start' , '<=' , $end->copy()->format( 'Y-m-d' ) )->orderBy( 'start' , 'ASC' )->get();

				$gastos = \App\Expenses::where( 'date' , '>=' , $start->copy()->format( 'Y-m-d' ) )->Where( 'date' , '<=' , $end->copy()->format( 'Y-m-d' ) )->Where( 'PayFor' , 'LIKE' , '%' . $room . '%' )->orderBy( 'date' , 'DESC' )->get();
			}
			...
			return ['total' => $total , 'apto' => $apto , 'park' => $park , 'lujo' => $lujo , 'room' => $room , 'banco' => $banco , 'metalico' => $metalico , 'pagado' => $gastos->sum( 'import' ) ,];
		...
		static function getSalesByYearByRoomGeneral( $year = "" , $room = "all" )
		{
			...
			if ( $room == "all" ) {
				$rooms = \App\Rooms::where( 'state' , 1 )->get( ['id'] );
				$books = \App\Book::whereIn( 'type_book' , [2 , 7 , 8] )->whereIn( 'room_id' , $rooms )->where( 'start' , '>=' , $start->copy()->format( 'Y-m-d' ) )->where( 'start' , '<=' , $end->copy()->format( 'Y-m-d' ) )->orderBy( 'start' , 'ASC' )->get();
				...
				$gastos = \App\Expenses::where( 'date' , '>=' , $start->copy()->format( 'Y-m-d' ) )->Where( 'date' , '<=' , $start->copy()->addYear()->format( 'Y-m-d' ) )->orderBy( 'date' , 'DESC' )->get();
				...
				$books = \App\Book::whereIn( 'type_book' , [2 , 7 , 8] )->where( 'room_id' , $room )->where( 'start' , '>=' , $start->copy()->format( 'Y-m-d' ) )->where( 'start' , '<=' , $end->copy()->format( 'Y-m-d' ) )->orderBy( 'start' , 'ASC' )->get();
				...
				$gastos = \App\Expenses::where( 'date' , '>=' , $start->copy()->format( 'Y-m-d' ) )->Where( 'date' , '<=' , $start->copy()->addYear()->format( 'Y-m-d' ) )->Where( 'PayFor' , 'LIKE' , '%' . $room . '%' )->orderBy( 'date' , 'DESC' )->get();
			...
			return ['total' => $total , 'banco' => $banco , 'metalico' => $metalico , 'pagado' => $pagado , 'metalico_jaime' => $metalico_jaime , 'metalico_jorge' => $metalico_jorge , 'banco_jorge' => $banco_jorge , 'banco_jaime' => $banco_jaime ,
		...
		public function getHojaGastosByRoom( $year = "" , $id )
		{
			...
			if ( $id != "all" ) {
				...
				$gastos = \App\Expenses::where( 'date' , '>=' , $start->copy()->format( 'Y-m-d' ) )->Where( 'date' , '<=' , $end->copy()->format( 'Y-m-d' ) )->Where( 'PayFor' , 'LIKE' , '%' . $id . '%' )->orderBy( 'date' , 'DESC' )->get();
			} else {
				$room   = "all";
				$gastos = \App\Expenses::where( 'date' , '>=' , $start->copy()->format( 'Y-m-d' ) )->Where( 'date' , '<=' , $end->copy()->format( 'Y-m-d' ) )->orderBy( 'date' , 'DESC' )->get();
			...
			return view( 'backend.sales.gastos._expensesByRoom' , ['gastos' => $gastos , 'room' => $room , 'year' => $start] );
		}
		...
		public function getTableExpensesByRoom( $year = "" , $id){
			...
			return view( 'backend.sales.gastos._tableExpensesByRoom' , ['gastos' => $gastos , 'room' => $room , 'year' => $start] );
		...
		public function searchByName( Request $request )
		{
			...
			if ( $request->searchString != "" ) {
				...
				if ( count( $customers ) > 0 ) {
					...
					return view( 'backend/sales/_tableSummary' , ['books' => $books , 'totales' => $totales , 'data' => $data , 'percentBenef' => DB::table( 'percent' )->find( 1 )->percent , 'temporada' => $date] );
				} else {
		...
		public function searchByRoom( Request $request )
		{
			...
			if ( $request->searchString != "" ) {
				...
				if ( count( $customers ) > 0 ) {
					...
					return view( 'backend/sales/_tableSummary' , ['temporada' => $date , 'books' => $books , 'totales' => $totales , 'data' => $data , 'percentBenef' => DB::table( 'percent' )->find( 1 )->percent ,] );
				} else {
					return "<h2>No hay reservas para este término '" . $request->searchString . "'</h2>";
				}
			}
				...
				if ( $request->searchRoom && $request->searchRoom != "all" ) {
					...
					$books = \App\Book::where( 'start' , '>=' , $date )->where( 'start' , '<=' , $date->copy()->addYear()->subMonth() )->whereIn( 'type_book' , [2 , 7] )->where( 'room_id' , $request->searchRoom )->orderBy( 'start' , 'ASC' )->get();
			...
			if ( $request->searchString != "" ) {
				...
				if ( count( $customers ) > 0 ) {
					...
					if ( $request->searchRoom && $request->searchRoom != "all" ) {
						...
						$books = \App\Book::whereIn( 'customer_id' , $arrayCustomersId )->where( 'start' , '>=' , $date->format( 'Y-m-d' ) )->where( 'start' , '<=' , $date->copy()->addYear()->subMonth()->format( 'Y-m-d' ) )->whereIn( 'type_book' , [2 , 7] )->where( 'room_id' , $request->searchRoom )->orderBy( 'inc_percent' , 'ASC' )->get();

						$diasPropios = \App\Book::whereIn( 'customer_id' , $arrayCustomersId )->where( 'start' , '>' , $date->copy()->subMonth() )->where( 'finish' , '<' , $date->copy()->addYear() )->whereIn( 'type_book' , [7 , 8] )->where( 'room_id' , $request->searchRoom )->orderBy( 'inc_percent' , 'ASC' )->get();

					} else {
						$books = \App\Book::whereIn( 'customer_id' , $arrayCustomersId )->where( 'start' , '>=' , $date->format( 'Y-m-d' ) )->where( 'start' , '<=' , $date->copy()->addYear()->subMonth()->format( 'Y-m-d' ) )->whereIn( 'type_book' , [2 , 7] )->orderBy( 'inc_percent' , 'ASC' )->get();

						$diasPropios = \App\Book::whereIn( 'customer_id' , $arrayCustomersId )->where( 'start' , '>' , $date->copy()->subMonth() )->where( 'finish' , '<' , $date->copy()->addYear() )->whereIn( 'type_book' , [7 , 8] )->orderBy( 'inc_percent' , 'ASC' )->get();
					}
				...
				if ( $request->searchRoom && $request->searchRoom != "all" ) {
					...
					$books = \App\Book::where( 'start' , '>=' , $date )->where( 'start' , '<=' , $date->copy()->addYear()->subMonth() )->whereIn( 'type_book' , [2 , 7] )->where( 'room_id' , $request->searchRoom )->orderBy( 'inc_percent' , 'ASC' )->get();

					$diasPropios = \App\Book::where( 'start' , '>' , $date->copy()->subMonth() )->where( 'room_id' , $request->searchRoom )->where( 'finish' , '<' , $date->copy()->addYear() )->whereIn( 'type_book' , [7 , 8] )->orderBy( 'inc_percent' , 'ASC' )->get();


				} else {
					$books = \App\Book::where( 'start' , '>=' , $date )->where( 'start' , '<=' , $date->copy()->addYear()->subMonth() )->whereIn( 'type_book' , [2 , 7] )->orderBy( 'inc_percent' , 'ASC' )->get();

					$diasPropios = \App\Book::where( 'start' , '>' , $date->copy()->subMonth() )->where( 'finish' , '<' , $date->copy()->addYear() )->whereIn( 'type_book' , [7 , 8] )->orderBy( 'inc_percent' , 'ASC' )->get();
