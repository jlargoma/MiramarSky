...
class XML_RPC
{
    ...
    public static function CallMethod($host, $method, $params)
    {
      ...
      $xmlrpc_useragent = 'CellSix/1.3';
        ...
        $payload = str_replace('temp-dataid-', '', $payload);
//        echo $payload; die;
        // create a stream HTTP resource for our POST
        
        
        $payload = str_replace('_int_','', $payload);
        
        $xml_rpc = stream_context_create(array(
            'http' => array(
                'method' => "POST",
                'header' => "Content-Type: text/xml\r\n" .
                            "User-Agent: " . $xmlrpc_useragent . "\r\n",
                'content' => $payload
            )
        ));
//print($payload); die;
        // HTTP response as raw data
//        $raw_data = file_get_contents(self::Host($host) . ":" . $xmlrpc_port . $xmlrpc_path, false, $xml_rpc);
        
        $raw_data = file_get_contents(self::Host($host) . $xmlrpc_path, false, $xml_rpc);
