    ...
    /*******************************************/
      ...
      if ($books){
        foreach ($books as $b){
          ...
          if ($country && !($country == 'ES'|| $country == 'es' || $country == '--Seleccione país --')){
            if (!isset($list_recidencias[$country])) $list_recidencias[$country] = $aLstDays;
          }
          
          if ( ($country == 'ES' || $country == 'es') && $province){
...
//echo '<br/>'.array_sum($auxTemp['direct']) / count($auxTemp['direct']);
      ...
      foreach ($list_recidencias as $country => $lst_by_prov){
          ...
          if (is_numeric($country)){
            $RESIDENCIAS[] = [
             ...
             $RESIDENCIAS[] = [
              'ID_PAIS' => isset($iso3Country[$country]) ? $iso3Country[$country] : '',
    ...
    if ($adr['indiv']['c_agency']>0){
       $TARIFA_TOUROPERADOR = round($adr['indiv']['agency']/$adr['indiv']['c_agency'],2);
       $PCTN_TARIFA_TOUROPERADOR = round($adr['indiv']['c_agency']/$PCTN_total*100);
    }
    
    if ($adr['indiv']['c_direct']>0){
       $TARIFA_NORMAL = round($adr['indiv']['direct']/$adr['indiv']['c_direct'],2);
       $PCTN_TARIFA_NORMAL = 100-$PCTN_TARIFA_TOUROPERADOR;
//       $PCTN_TARIFA_NORMAL = round($adr['indiv']['c_direct']/$PCTN_total*100);
    }
    $obj['PRECIOS']['APARTAMENTOS_2-4pax']['TARIFA_NORMAL'] = $TARIFA_NORMAL;
    $obj['PRECIOS']['APARTAMENTOS_2-4pax']['PCTN_TARIFA_NORMAL'] = $PCTN_TARIFA_NORMAL;
    $obj['PRECIOS']['APARTAMENTOS_2-4pax']['TARIFA_TOUROPERADOR'] = $TARIFA_TOUROPERADOR;
    $obj['PRECIOS']['APARTAMENTOS_2-4pax']['PCTN_TARIFA_TOUROPERADOR'] = $PCTN_TARIFA_TOUROPERADOR;
    ...
    if ($adr['doble']['c_agency']>0){
       $TARIFA_TOUROPERADOR = round($adr['doble']['agency']/$adr['doble']['c_agency'],2);
       $PCTN_TARIFA_TOUROPERADOR = round($adr['doble']['c_agency']/$PCTN_total*100);
    }
    
    if ($adr['doble']['c_direct']>0){
       $TARIFA_NORMAL = round($adr['doble']['direct']/$adr['doble']['c_direct'],2);
//       $PCTN_TARIFA_NORMAL = round($adr['doble']['c_direct']/$PCTN_total*100);
       $PCTN_TARIFA_NORMAL = 100-$PCTN_TARIFA_TOUROPERADOR;
    }
    $obj['PRECIOS']['APARTAMENTOS_4-6pax']['TARIFA_NORMAL'] = $TARIFA_NORMAL;
    $obj['PRECIOS']['APARTAMENTOS_4-6pax']['PCTN_TARIFA_NORMAL'] = $PCTN_TARIFA_NORMAL;
    $obj['PRECIOS']['APARTAMENTOS_4-6pax']['TARIFA_TOUROPERADOR'] = $TARIFA_TOUROPERADOR;
    $obj['PRECIOS']['APARTAMENTOS_4-6pax']['PCTN_TARIFA_TOUROPERADOR'] = $PCTN_TARIFA_TOUROPERADOR;
  ...
  function getEncuestaStructApartamento(){
    ...
    'APARTAMENTOS' =>[
      ...
      'CABECERA_APARTAMENTOS' =>['CATEGORIA'=>0],
      'INFORMANTE'=>[
          'NOMBRE' => 'Jorge Largo',
          'CARGO' => 'Gerente',
