...
//    $days  = cal_days_in_month(CAL_GREGORIAN, $month, $year);
    ...
    $obj['CABECERA']['CATEGORIA'] = 'H1';
    $obj['CABECERA']['HABITACIONES'] = count($roomsID);
    $obj['CABECERA']['PLAZAS_DISPONIBLES_SIN_SUPLETORIAS'] = Rooms::where('state',1)->where('site_id',$site_id)->sum('minOcu');
      ...
      foreach ($list_recidencias as $country => $lst_by_prov){
          ...
          if (is_numeric($country)){
            $RESIDENCIAS[] = [
             ...
             $RESIDENCIAS[] = [
              'ID_PAIS' => isset($iso3Country[$country]) ? $iso3Country[$country] : $country.'',
    ...
    if ($adr['c_agency']>0){
       ...
       $PCTN_HABITACIONES_OCUPADAS_AGENCIA_ONLINE = round($adr['c_agency']/$PCTN_total*100);
    }
    
    if ($adr['c_direct']>0){
       $ADR_INTERNET = round($adr['direct']/$adr['c_direct'],2);
       $PCTN_HABITACIONES_OCUPADAS_INTERNET = round($adr['c_direct']/$PCTN_total*100);
    }
