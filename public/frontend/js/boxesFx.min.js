...
BoxesFx.prototype._navigate=function(dir){if(this.isAnimating)return false;this.isAnimating=true;var self=this,currentPanel=this.panels[this.current];if(dir==='next'){this.current=this.current<this.slidesCount-1?this.current+1:0;}
else{this.current=this.current>0?this.current-1:this.slidesCount-1;}
var nextPanel=this.panels[this.current];classie.add(nextPanel,'active');this._applyTransforms(currentPanel,dir);var cntTransTotal=0,onEndTransitionFn=function(ev){if(ev&&!classie.has(ev.target,'bg-img'))return false;++cntTransTotal;if(cntTransTotal<self.slidesCount)return false;if(support.transitions){this.removeEventListener(transEndEventName,onEndTransitionFn);}
