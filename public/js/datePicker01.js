$(document).ready(function () {
    ...
    if (typeof startDate != 'undefined') {
        ...
        dateRangeObj.startDate = new Date(startDate);
        dateRangeObj.endDate = new Date(endDate);
        $(".daterange03").daterangepicker(dateRangeObj);
    }
    ...
    $('.daterange03').on('change', function (event) {
        var date = $(this).val();
        var arrayDates = date.split('-');
        var res1 = arrayDates[0].replace("Abr", "Apr");
        var date1 = new Date(res1);
        ...
        var content = $(this).closest('div');
        var startContent = content.find('.filter_startDate');
        if (startContent)
            startContent.val(date1.yyyymmmdd());
        var endContent = content.find('.filter_endDate');
        if (endContent)
            endContent.val(date2.yyyymmmdd());
