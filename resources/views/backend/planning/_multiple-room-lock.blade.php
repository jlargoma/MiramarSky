...
<div style="max-width: 480px;margin: auto;padding: 1em;">
  ...
  <h4 class="text-center">Bloqueo de Alojamientos</h4>
  <div class="row">
    <div class="col-md-4 col-xs-12">
      <label>Alojamiento</label>
      <select id="mrlRoom" class="form-control">
        <option value="-1"></option>
        <?php foreach ($oRooms as $k => $v): ?>
  ...
  <div class="form-group row">
    <label for="static" class="col-sm-5 col-form-label">Alojamientos</label>
    <div class="col-sm-7">
      <?php foreach ($oRooms as $k => $v): ?>
        <?php $checked = $aTaskData['rooms'][$k] ? 'checked' : ''; ?>
        <div class="form-check">
          <input type="checkbox" class="form-check-input checkRooms" data-id="<?php echo $k; ?>" <?php echo $checked; ?>>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('#mrlSendProgr').on('click', function () {
      ...
      $( ".checkRooms" ).each(function( index ) {
      ...
      if (rooms.length < 1) {
        alert('seleccione un alojamiento para bloquear');
        return null;
      }
