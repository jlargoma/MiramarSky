    ...
    <div class="row">
        <table class="table  table-condensed table-striped table-data" data-type="confirmadas" style="margin-top: 0;">
            ...
            <tr>
                ...
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 4%!important">&nbsp;
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 11%!important"> Cliente
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 10%!important"> Telefono
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 7%!important"> Pax
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 5%!important"></th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 10%!important"> Apart
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 5%!important"><i class="fa fa-moon-o"></i></th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 6%!important"> IN
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 8%!important"> OUT
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 17%!important"> Precio
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 50px!important"> &nbsp;
                </th>
                <th class="text-center @if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white"
                    style="width: 17%!important"> Estado
                </th>
            </tr>
            </thead>
            <tbody>
            ...
            <tr>
                ...
                <td class="text-center" style="padding: 10px 15px!important">
                    <a class="update-book" data-id="<?php echo $book->id ?>"
                       ...
                       <?php if (isset($payment[$book->id])): ?>  style="color: red" <?php endif ?>
                ...
                <td class="text-center">
                    ...
                    <a href="tel:<?php echo $book->customer->phone ?>"><?php echo $book->customer->phone ?>
						<?php else: ?>
                        <input type="text" class="only-numbers customer-phone"
                ...
                <td class="text-center">
                    ...
                    <button class="font-w800 btn btn-xs getImagesCustomer" type="button" data-toggle="modal"
                            data-target="#modalRoomImages"
                            style="z-index: 99; border: none; background-color:transparent!important; color: lightgray; padding: 0;"
                    ...
                    <button class="font-w800 btn btn-xs getImagesCustomer" type="button" data-toggle="modal"
                            data-target="#modalRoomImages"
                            style="z-index: 99; border: none; background-color: transparent!important; color:black; padding: 0;"
                            data-id="<?php echo $book->room->id ?>"
                            data-idCustomer="<?php echo $book->id ?>"
                    >
                        <i class="fa fa-eye"></i>
...
<div class="table-responsive" style="border: none!important">
    <table class="table table-striped" style="margin-top: 0;">
        ...
        <th class="@if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white text-center">Nombre
        </th>
        <th class="@if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white text-center"
            style="min-width:50px">In
        </th>
        <th class="@if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white text-center"
            style="min-width:50px ">Out
        </th>
        ...
        <th class="@if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white text-center"
            style="min-width:100px">Apart
        </th>
        <th class="@if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white text-center"><i
                    class="fa fa-moon-o"></i></th>
        <th class="@if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-white text-center"
            style="min-width:65px">PVP
        </th>
        <th class="@if($type == 'confirmadas')Pagada-la-señal @else blocked-ical @endif text-center  text-white"
            style="width: 50px!important"> &nbsp;
        </th>
        ...
        <tr class="<?php echo ucwords($book->getStatus($book->type_book));?>">
            ...
            <td class="text-center">
				<?php if ($book->agency != 0): ?>
                <img style="width: 15px;margin: 0 auto;"
                     src="/pages/<?php echo strtolower($book->getAgency($book->agency)) ?>.png" align="center"/>
				<?php endif ?>
            </td>
            <td class="text-left">
            ...
            <td class="text-center"><?php echo Carbon::CreateFromFormat('Y-m-d', $book->start)
			                                         ->formatLocalized('%d %b') ?></td>
            <td class="text-center"><?php echo Carbon::CreateFromFormat('Y-m-d', $book->finish)
			                                         ->formatLocalized('%d %b') ?></td>
            <td class="text-center">
				<?php if ($book->real_pax > 6 ): ?>
				<?php echo $book->real_pax ?><i class="fa fa-exclamation" aria-hidden="true" style="color: red"></i>
				<?php elseif($book->pax != $book->real_pax): ?>
				<?php echo $book->real_pax ?><i class="fa fa-exclamation-circle" aria-hidden="true"
                                                style="color: red"></i>
				<?php else: ?>
                            <?php echo $book->pax ?>
                        <?php endif ?>
                ...
                <select class="status form-control minimal" data-id="<?php echo $book->id ?>">
	                ...
	                <?php else: ?>
                        <?php for ($i=1; $i <= count($status); $i++): ?>
                            <?php if ($i == 5 && $book->customer->email == ""): ?> <?php else: ?>
                            <option <?php echo $status[$i] == ($book->type_book) ? "selected" : ""; ?> <?php
                                    echo ($status[$i]  == 1 || $status[$i] == 5) ? "style='font-weight:bold'" : "" ?> value="<?php echo $status[$i] ?>"  data-id="<?php echo $book->id ?>">
                                <?php echo $book->getStatus($status[$i]) ?>
                            </option>
                            <?php endif ?>
                        <?php endfor; ?>
